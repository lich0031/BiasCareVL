...
def get_points_from_mask_batch(masks, get_point=1, top_num=0.5):
    """
    Process masks in batch mode and return one sampled point and label for each mask.
    masks: shape (B, H, W) or (B, 1, H, W)
    """
    ...
    # Remove the extra dimension if the mask has one.
    for mask in masks:
        if len(mask.shape) > 2:
            mask = mask.squeeze()
        ...
        # If the foreground is empty, sample directly from the background.
        if len(fg_coords) == 0:
            coord = bg_coords[np.random.randint(len(bg_coords))]
            label = 0
        else:
            ...
            distances = np.sqrt(np.sum((fg_coords - centroid) ** 2, axis=1))
            sorted_indices = np.argsort(distances)
            top_k = int(len(fg_coords) * top_num)
            top_k = top_k if top_k > 0 else 1
            top_indices = sorted_indices[:top_k]
            try:
                coord = fg_coords[np.random.choice(top_indices, size=get_point)[0]]  # Sample a single point only.
                label = 1
            except Exception:
                coord = bg_coords[np.random.randint(len(bg_coords))]
                label = 0

        coords_batch.append(coord.tolist())
        labels_batch.append(label)

    coords = torch.as_tensor(coords_batch, dtype=torch.float).unsqueeze(1)
    labels = torch.as_tensor(labels_batch, dtype=torch.int).unsqueeze(1)
    return coords, labels
...
def get_bboxes_from_mask(masks, offset=0):
    ...
    return torch.tensor(bounding_boxes, dtype=torch.int).unsqueeze(1)
